...
const paths = realPaths.split("\n").map((x) => x.split(","));

// Traverse the first path and populate a map keyed by coordinates.
// Then traverse the second path and create a new map, removing items from first map as you find them.
...
const run = () => {
  ...
  paths[0].forEach((path) => {
    let dir = path[0];
    let dist = parseInt(path.slice(1), 10);
    for (let i = 0; i < dist; i++) {
      ...
      // Don't re-add cells we already have visited
  ...
  paths[1].forEach((path) => {
    ...
    for (let i = 0; i < dist; i++) {
      ...
      if (map[`${x},${y}`]) {
        intersections.push({ x, y, steps: steps + map[`${x},${y}`] });
  ...
  return intersections.sort((a, b) => a.steps - b.steps)[0].steps;
};
